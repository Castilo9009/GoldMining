...
// CityDistance calcuates the total distance from start to stop (not length of path)
...
// HorizDistance calculates the horizontal portion of the distance
func (p *Path) HorizDistance() int64 {
	return int64(p.Steps[0].Col - p.Dest().Col)
}

// VertDistance calculates the vertical portion of the distance
